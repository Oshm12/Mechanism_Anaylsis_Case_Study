...
%%OPTIMSATION CODE - CYCLE THROUGH NUMBERS AND EVALUATE
for i = 1:length(z0)
                    ...
                    if ~any(ex < -100)
                        ...
                        if ~any(ex > 100)
                            ...
                            if percentage_unused < percentage_unused_max
                                %if all constaints ok and bigger than
                                %previous max, set as new value
...
%%optimsation done
%repeat kinematics with new values
